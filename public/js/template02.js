/* =============================================================
   template02.js — Google SWE Live Preview + Load on Edit
   ============================================================= */
...
function syncTagline()  { targetTagline.textContent = details.value.trim()  || 'Software Engineer · Mountain View, CA'; }
function syncSummary()  { targetSummary.textContent = summary.value.trim()  || 'Software engineer with experience building distributed systems at scale.'; }

function syncContact() {
  targetPhone.textContent    = ph.value.trim()       || '+1 650 555 0100';
  targetEmail.textContent    = mail.value.trim()     || 'jane@gmail.com';
...
function syncSkills() {
  ...
  if (!val) {
    targetSkills.innerHTML = '<span class="g-chip">Go</span><span class="g-chip">Python</span><span class="g-chip">Distributed Systems</span>';
    return;
  }
  targetSkills.innerHTML = val
    ...
    .map(s => s.trim())
    .filter(Boolean)
    .map(s => `<span class="g-chip">${s}</span>`)
    .join('');
}
...
function buildExpItem({ title, company, start, end, desc }) {
  const lines = desc ? desc.split('\n').filter(l => l.trim()) : ['Describe impact with metrics'];
  const date  = [start, end].filter(Boolean).join(' – ');
  const div   = document.createElement('div');
  div.className = 'g-entry expItem';
  div.innerHTML = `
    <div class="g-entry-top">
      <div class="g-entry-org expTitle">${title || 'Software Engineer'}</div>
      ${date ? `<div class="g-entry-date expDuration">${date}</div>` : ''}
    </div>
    <div class="g-entry-role expCompany">${company || ''}</div>
    <ul class="g-entry-desc expDesc">
...
function buildProjItem({ name, tech, link, desc }) {
  ...
  const div   = document.createElement('div');
  div.className = 'g-entry projItem';
  div.innerHTML = `
    <div class="g-entry-top">
      <div class="g-entry-org projName">${name || 'Project'}</div>
    </div>
    ${tech ? `<div class="g-entry-role projTech">${tech}</div>` : ''}
    ${link ? `<div class="g-entry-link projLink">${link}</div>` : ''}
    <ul class="g-entry-desc projDesc">
      ${lines.map(l => `<li>${l}</li>`).join('')}
    </ul>`;
  return div;
}
...
function buildEduItem({ degree, institute, start, end, desc }) {
  ...
  div.innerHTML = `
    <div class="g-entry-top">
      <div class="g-entry-org expTitle">${institute || 'University'}</div>
      ${date ? `<div class="g-entry-date expDuration">${date}</div>` : ''}
    </div>
    <div class="g-entry-role expCompany">${degree || ''}</div>
    ${lines.length ? `<ul class="g-entry-desc expDesc">${lines.map(l => `<li>${l}</li>`).join('')}</ul>` : ''}`;
  return div;
}
...
/* ================================================================
   LOAD SAVED RESUME ON EDIT (?resumeId= in URL)
   ================================================================ */
